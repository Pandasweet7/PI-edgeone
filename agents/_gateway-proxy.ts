...
/**
 * The Makers AI Gateway is OpenAI-compatible; pi's OpenAI-compatible transport
 * sends a `developer` role for reasoning-capable models, which some gateway
 * vendors reject. Normalize it to `system` like the DSH template does.
 */
...
async function proxyGatewayRequest(
  ...
  if (!upstreamBaseUrl || !apiKey) {
    ...
    response.end(JSON.stringify({ error: 'AI Gateway environment is incomplete: set AI_GATEWAY_API_KEY and AI_GATEWAY_BASE_URL' }))
    return
  }
