...
export function isPluginBackendOperation(value) {
    return value.length <= PLUGIN_BACKEND_OPERATION_MAX_LENGTH && OPERATION_PATTERN.test(value);
}
export function requirePluginBackendOperation(value) {
...
export function requirePluginBackendRevision(value) {
...
export function cloneBoundedPluginBackendJson(value, label, maxBytes = PLUGIN_BACKEND_JSON_MAX_BYTES) {
    const cloned = cloneJsonValue(value, new Set(), label, 0);
...
export function serializeBoundedPluginBackendJson(value, label, maxBytes = PLUGIN_BACKEND_JSON_MAX_BYTES) {
    return JSON.stringify(cloneBoundedPluginBackendJson(value, label, maxBytes));
}
export function parseBoundedPluginBackendJson(text, label, maxBytes = PLUGIN_BACKEND_JSON_MAX_BYTES) {
...
export function parsePluginBackendRequestEnvelope(value) {
    if (!isPlainRecord(value))
        throw new Error("Plugin backend request body must be an object");
...
function cloneJsonValue(value, ancestors, label, depth) {
    if (depth > MAX_JSON_DEPTH)
        throw new Error(`${label} exceeds the maximum JSON depth of ${String(MAX_JSON_DEPTH)}`);
...
//# sourceMappingURL=pluginBackendProtocol.js.map