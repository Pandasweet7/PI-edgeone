...
export function parsePiWebVersionResponse(value) {
    if (!isRecord(value))
        return undefined;
    const packageName = value["packageName"];
    const generatedAt = value["generatedAt"];
    const components = value["components"];
    if (typeof packageName !== "string" || packageName === "" || typeof generatedAt !== "string" || generatedAt === "" || !isRecord(components))
        return undefined;
    const web = parsePiWebComponentStatus(components["web"]);
    const sessiond = parsePiWebComponentStatus(components["sessiond"]);
    if (web === undefined || sessiond === undefined)
        return undefined;
    return { packageName, generatedAt, components: { web, sessiond } };
}
export function parsePiWebRuntimeResponse(value) {
    if (!isRecord(value))
        return undefined;
    const packageName = value["packageName"];
    const generatedAt = value["generatedAt"];
    ...
    if (typeof packageName !== "string" || packageName === "" || typeof generatedAt !== "string" || generatedAt === "" || !isRecord(components) || capabilities === undefined)
        return undefined;
...
export function parsePiWebRuntimeComponent(value) {
    if (!isRecord(value))
        return undefined;
    ...
    if (component !== "web" && component !== "sessiond")
        return undefined;
    if (typeof label !== "string" || label === "" || typeof available !== "boolean" || capabilities === undefined)
        return undefined;
    if (activeAgentProfileValue !== undefined && (component !== "sessiond" || activeAgentProfile === undefined))
        return undefined;
    return {
        component,
        label,
 ...
 */
export function parseDeprecatedAgentInputs(value) {
    ...
    for (const entry of value) {
        ...
        if (source !== "environment" && source !== "config")
            continue;
        if (typeof name !== "string" || name === "")
            continue;
        if (replacement !== undefined && (typeof replacement !== "string" || replacement === ""))
            continue;
        inputs.push({ source, name, ...(replacement === undefined ? {} : { replacement }) });
    }
    return inputs;
}
export function parsePiWebComponentStatus(value) {
    if (!isRecord(value))
        return undefined;
    const component = value["component"];
    const label = value["label"];
...
export function parsePiWebInstallationInfo(value) {
    if (!isRecord(value))
        return undefined;
    ...
    if (kind !== "pi-package" && kind !== "npm-global" && kind !== "local" && kind !== "docker" && kind !== "unknown")
        return undefined;
    return {
        kind,
...
//# sourceMappingURL=piWebStatusParsing.js.map