...
export function parseWorkspaceRemovalRequest(value) {
    if (!isRecord(value))
        throw new Error("Workspace removal request must be an object");
    return { precondition: requireWorkspaceRemovalPrecondition(value["precondition"]) };
}
export function requireWorkspaceRemovalPrecondition(value) {
    if (typeof value !== "string"
        || value === ""
        || value.length > WORKSPACE_REMOVAL_PRECONDITION_MAX_LENGTH) {
        throw new Error(`Workspace removal precondition must be a non-empty string of at most ${String(WORKSPACE_REMOVAL_PRECONDITION_MAX_LENGTH)} characters`);
    }
    return value;
}
...
//# sourceMappingURL=workspaceRemovalProtocol.js.map