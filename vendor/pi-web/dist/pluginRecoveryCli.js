...
import { disableServerPlugin, loadServerPluginRecoveryConfig, setServerPluginSafeStart, } from "./serverPluginRecovery.js";
export function pluginRecoveryHelp() {
...
export function runPluginRecoveryCli(args, dependencies) {
...
function parsePluginRecoveryCommand(args) {
    const parsed = parseOptions(args);
    if (parsed.help)
        return { kind: "help" };
    const [command, ...rest] = parsed.positional;
    if (command === undefined || command === "help")
        return requireNoMutationOptions(parsed, { kind: "help" });
    if (command === "disable") {
        const [pluginId, ...extra] = rest;
        if (pluginId === undefined || extra.length > 0)
            throw new Error("Usage: pi-web plugins disable <plugin-id> [--config <path>] [--restart]");
    ...
    if (command !== "safe-start")
        throw new Error(`Unknown plugins command: ${command}`);
    const [action, value, ...extra] = rest;
    if (action === "show" && value === undefined && extra.length === 0) {
        if (parsed.restart)
            throw new Error("--restart is not valid with safe-start show");
...
function parseOptions(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "--help" || arg === "-h") {
            ...
            if (restart)
                throw new Error("Duplicate plugins option: --restart");
            restart = true;
        }
        else if (arg === "--config") {
            if (configPath !== undefined)
                throw new Error("Duplicate plugins option: --config");
...
function recoveryConfigOptions(configPath, dependencies) {
    return {
...
function withConfigPath(value, configPath) {
    return { ...value, ...(configPath === undefined ? {} : { configPath }) };
}
function requireNoMutationOptions(parsed, command) {
    if (parsed.configPath !== undefined || parsed.restart)
        throw new Error("Plugin recovery help does not accept --config or --restart");
    return command;
}
function restartPlanFor(configPath, explicitConfigPath, dependencies) {
    return dependencies.restartPlan({ configPath, explicitConfigPath: explicitConfigPath !== undefined });
}
function reportRecoveryDiagnostic(diagnostic, writeLine) {
    if (diagnostic !== undefined)
        writeLine(`Warning: ${diagnostic}`);
}
function reportRestart(performRestart, plan, writeLine) {
    ...
    if (plan.command !== undefined)
        writeLine(`  ${plan.command}`);
}
//# sourceMappingURL=pluginRecoveryCli.js.map