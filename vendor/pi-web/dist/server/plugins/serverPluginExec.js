...
export function createServerPluginExecFile(options = {}) {
...
function runExecFile(request, options) {
...
function validateRequest(request) {
    if (typeof request.file !== "string" || request.file === "")
        throw new Error("Server plugin command file must be a non-empty string");
    ...
    if (!isAbortSignal(request.signal))
        throw new Error("Server plugin commands require an AbortSignal");
}
function commandEnvironment(baseEnv, request) {
    const env = { ...baseEnv, ...(request.env ?? {}) };
    for (const key of request.unsetEnv ?? [])
        Reflect.deleteProperty(env, key);
    return env;
}
function isEnvironmentKey(value) {
...
//# sourceMappingURL=serverPluginExec.js.map