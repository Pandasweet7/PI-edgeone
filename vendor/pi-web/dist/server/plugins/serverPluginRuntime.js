 ...
 */
export async function createServerPluginRuntime(options) {
...
export class ServerPluginRuntime {
    constructor(safeStart, diagnostics, logger, importer, execFile, lifecycleTimeoutMs) {
    ...
    static async activate(snapshot, options) {
        const runtime = new ServerPluginRuntime(options.safeStart, Object.freeze(snapshot.diagnostics.map((diagnostic) => Object.freeze({ ...diagnostic }))), options.logger, options.importer ?? importServerPluginModule, options.execFile ?? createServerPluginExecFile(), positiveInteger(options.lifecycleTimeoutMs, DEFAULT_LIFECYCLE_TIMEOUT_MS, "lifecycleTimeoutMs"));
        try {
    ...
    async inspectHealth() {
        ...
        for (const active of this.activePlugins) {
            ...
            try {
                const result = await runBounded(active.entry.id, "health", this.lifecycleTimeoutMs, (signal) => callback(signal));
    ...
    async stop() {
        ...
        for (const active of activePlugins) {
            ...
            if (stop === undefined)
                continue;
            try {
                await runBounded(active.entry.id, "stop", this.lifecycleTimeoutMs, (signal) => stop(signal));
            }
            catch (error) {
    ...
    async activateEntry(entry) {
        ...
        try {
            ...
            const state = error instanceof IncompatibleServerPluginError ? "incompatible" : "failed";
    ...
    async rollbackStart(pluginId, activation) {
...
function disabledReason(entry, safeStart) {
    if (!entry.enabled)
        return "disabled in PI WEB config";
    if (safeStart === "none")
        return "disabled by no-server-plugin safe start";
    if (safeStart === "bundled-only" && entry.scope !== "bundled")
        return "disabled by bundled-only safe start";
    return undefined;
}
function recordFor(entry, status) {
...
function requireServerModule(entry) {
    const serverModule = entry.serverModule;
    if (serverModule === undefined)
        throw new Error(`PI WEB plugin has no server module: ${entry.id}`);
    return serverModule;
}
function serverModuleUrl(entry) {
...
async function importServerPluginModule(moduleUrl) {
    const imported = await import(moduleUrl);
    return imported;
}
function parsePluginExport(imported) {
    if (!isRecord(imported))
        throw new IncompatibleServerPluginError("Server plugin module must export a default plugin object");
    ...
    return Object.freeze({ apiVersion: 1, name: candidate.name, activate: (context) => activate(context) });
}
function isPiWebServerPlugin(value) {
...
function parseActivation(value) {
    if (!isRecord(value))
        throw new IncompatibleServerPluginError("Server plugin activation must be an object");
    ...
    for (const callback of ["start", "stop", "health"]) {
    ...
    if (!isServerPluginActivation(candidate))
        throw new IncompatibleServerPluginError("Server plugin activation is invalid");
    ...
    return Object.freeze({
        ...
        ...(start === undefined ? {} : { start: (signal) => start(signal) }),
        ...(stop === undefined ? {} : { stop: (signal) => stop(signal) }),
        ...(health === undefined ? {} : { health: (signal) => health(signal) }),
    });
}
function isServerPluginActivation(value) {
    if (!isRecord(value))
        return false;
...
function snapshotWorkspaceProvider(value) {
    if (!isRecord(value))
        throw new IncompatibleServerPluginError("Server plugin workspaceProvider is invalid");
    const candidate = {
    ...
    if (!isWorkspaceProvider(candidate))
        throw new IncompatibleServerPluginError("Server plugin workspaceProvider is invalid");
    ...
    return Object.freeze({
        ...
        probe: (project, signal) => probe(project, signal),
        list: (project, signal) => list(project, signal),
        ...(request === undefined ? {} : { request: (context) => request(context) }),
        ...(prepareRemove === undefined ? {} : { prepareRemove: (context) => prepareRemove(context) }),
    });
}
function isWorkspaceProvider(value) {
    if (!isRecord(value))
        return false;
...
function parseHealth(value) {
    if (!isRecord(value))
        throw new IncompatibleServerPluginError("Server plugin health must be an object");
...
function createScopedLogger(pluginId, logger) {
    return Object.freeze({
...
async function runBounded(pluginId, phase, timeoutMs, operation) {
    ...
    try {
        ...
        if (!controller.signal.aborted)
            controller.abort(new DOMException("Server plugin operation completed", "AbortError"));
    }
}
function cloneJsonObject(value, label) {
    if (!isRecord(value))
        throw new IncompatibleServerPluginError(`${label} must be a JSON object`);
    return cloneJsonRecord(value, new Set(), label);
}
function cloneJsonRecord(value, ancestors, label) {
    if (ancestors.has(value))
        throw new IncompatibleServerPluginError(`${label} must not contain cycles`);
    ancestors.add(value);
    const output = {};
    for (const [key, child] of Object.entries(value))
        output[key] = cloneJsonValue(child, ancestors, label);
...
function cloneJsonValue(value, ancestors, label) {
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            throw new IncompatibleServerPluginError(`${label} must contain only finite JSON numbers`);
        return value;
    }
...
class IncompatibleServerPluginError extends Error {
    constructor() {
        super(...arguments);
        this.name = "IncompatibleServerPluginError";
    }
}
...
//# sourceMappingURL=serverPluginRuntime.js.map