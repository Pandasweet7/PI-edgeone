...
export class SessiondStateOwnershipConflictError extends Error {
    constructor(options) {
        ...
        super(`another pi-web session daemon already owns this state\n\n` +
 ...
 * the grace window.
 */
export async function claimSessiondStateOwnership(options) {
    ...
    for (;;) {
        ...
        if (existing !== undefined && existing !== "invalid") {
            if (existing.pid !== ownPid && isAlive(existing)) {
                ...
                if (!waitedForOwner) {
                    logger?.warn({ markerPath, owner: existing }, "another live session daemon owns this state; waiting briefly for it to release ownership");
                    waitedForOwner = true;
                }
            ...
            if (existing.pid !== ownPid) {
                logger?.info({ markerPath, previousOwner: existing }, "taking over session daemon state ownership from a stale marker");
            }
...
export function parseOwnershipRecord(content) {
    try {
 ...
 */
export function isOwnershipRecordAlive(record) {
...
async function readOwnershipRecord(markerPath) {
    let content;
    try {
...
async function releaseOwnershipMarker(markerPath, record, logger) {
    try {
        const current = await readOwnershipRecord(markerPath);
        if (current === undefined || current === "invalid")
            return;
...
//# sourceMappingURL=sessiondStateOwnership.js.map