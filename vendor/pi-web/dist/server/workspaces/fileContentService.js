...
import { classifyWorkspaceFile, MAX_WORKSPACE_FILE_CONTENT_BYTES } from "../../shared/workspaceFiles.js";
...
export async function writeWorkspaceFile(rootPath, path, content, options = {}) {
    ...
    try {
        ...
        if (isNodeErrorWithCode(error, "ENOENT")) { /* expected for creation — continue */ }
            ...
            throw error; // re-throw permission errors, "not a file", traversal errors, etc.
    }
...
export async function deleteWorkspaceFile(rootPath, path) {
    if (path === undefined || path === "")
        throw new Error("path query parameter is required");
    ...
    try {
        ...
        if (s.isDirectory())
            throw new Error("Path is a directory, use directory deletion instead");
...
export async function moveWorkspaceFile(rootPath, fromPath, toPath, options = {}) {
    if (fromPath === undefined || fromPath === "")
        throw new Error("fromPath query parameter is required");
    if (toPath === undefined || toPath === "")
        throw new Error("toPath query parameter is required");
    const createDirs = options.createDirs ?? true;
...
function mediaForClassification(classification) {
