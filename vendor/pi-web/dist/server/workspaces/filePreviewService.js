...
export async function readWorkspaceFilePreview(rootPath, path, pathAccess, options = {}) {
    ...
    try {
        ...
        if (options.download === true) {
            if (stats.size === 0)
                return { ...metadata, size: 0, body: Buffer.alloc(0) };
        ...
        if (stats.size > MAX_INLINE_PREVIEW_BYTES)
            throw new Error(`File is too large to preview (limit ${MAX_INLINE_PREVIEW_LABEL})`);
 ...
 */
async function readValidatedSnapshot(handle, size) {
    if (size === 0)
 ...
 */
function validatedFileStream(handle, size) {
    ...
    return pipeline(source, exactLength, () => { });
}
//# sourceMappingURL=filePreviewService.js.map