...
const daemonEnvironment = Object.freeze({ ...process.env });
...
if (serverPluginRecovery.safeStartDiagnostic !== undefined) {
    app.log.error({ component: "server-plugins", configPath: serverPluginRecovery.path }, serverPluginRecovery.safeStartDiagnostic);
}
...
async function createSessionDaemonRuntime() {
    ...
    try {
        ...
        const workspaceProviderRuntime = createWorkspaceProviderRuntimeSnapshot(serverPlugins.healthRecords(), providerHealth, serverPlugins.safeStartLevel(), serverPlugins.catalogDiagnostics());
...
function registerSessionDaemonRoutes({ eventHub, machineStatus, statusAttribution, auth, sessions, terminals, runtimeComponent, projects, workspaceProviders, workspaceProviderRuntime, workspaceRemovals }) {
...
async function listenSessionDaemon({ shutdown }) {
...
//# sourceMappingURL=sessiond.js.map