...
import { CORE_STATUS_FLAGS, rollUpStatusFlags, } from "../../shared/machineStatus.js";
/**
...
export class MachineStatusService {
    ...
    async publishIfChanged() {
        ...
        if (isSameStatusTree(tree, this.current))
            return;
        this.current = {
     ...
     */
    flagsByCwd() {
        const flagsByCwd = new Map();
        const setFlag = (cwd, flagId) => {
            if (cwd === "")
                return;
        ...
        for (const activity of this.dependencies.activity.snapshot().workspaces) {
            if (activity.hasSessionActivity)
                setFlag(activity.cwd, CORE_STATUS_FLAGS.working);
            if (activity.hasTerminalActivity)
                setFlag(activity.cwd, CORE_STATUS_FLAGS.terminal);
        }
...
class FlagAccumulator {
    ...
    rollUp() {
        const nodes = new Map();
        for (const [nodeId, sources] of this.sourcesByNodeId)
            nodes.set(nodeId, rollUpStatusFlags(sources));
        return Object.fromEntries(nodes);
    }
}
function isSameStatusTree(left, right) {
...
function isSameNodes(left, right) {
...
function isSameFlags(left, right) {
    const leftIds = Object.keys(left);
    if (leftIds.length !== Object.keys(right).length)
        return false;
    return leftIds.every((flagId) => left[flagId] === right[flagId]);
}
//# sourceMappingURL=machineStatusService.js.map