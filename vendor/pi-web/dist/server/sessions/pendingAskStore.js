...
import { ASK_USER_ID_MAX_LENGTH, ASK_USER_OPTION_LIMIT, ASK_USER_OTHER_TEXT_MAX_LENGTH, ASK_USER_QUESTION_LIMIT, ASK_USER_TEXT_MAX_LENGTH, } from "../../shared/apiTypes.js";
...
export class PendingAskStore {
     ...
     */
    submit(sessionId, askId, submission) {
        const ask = this.openBySessionId.get(requireSessionId(sessionId));
        if (ask?.askId !== askId)
            return { status: "stale" };
    ...
    requireClose(sessionId, reason, answers) {
        const outcome = this.close(sessionId, reason, this.timestamp(), answers);
        if (outcome === undefined)
            throw new Error(`Pending ask of session ${sessionId} disappeared while closing`);
        return outcome;
    }
    close(sessionId, reason, closedAt, answers) {
 ...
 */
export function renderAskUserAnswersText(outcome) {
 ...
 */
export function renderSupersededAskText(outcome) {
    return [
...
function questionLines(record) {
    const header = `- ${record.question.id}: ${record.question.question}`;
    if (!record.answered)
        return [header, "  Unanswered."];
    const parts = [...record.values.map((value) => `selected ${value}`)];
    if (record.otherText !== undefined)
        parts.push(`custom: ${JSON.stringify(record.otherText)}`);
    return [header, `  Answered: ${parts.join("; ")}`];
}
function askUserOutcome(ask, answers, reason, closedAt) {
...
function questionRecord(question, answer) {
...
function summaryLine(total, answeredCount, unansweredIds) {
...
function validateQuestions(questions) {
    ...
    return questions.map((question) => {
        ...
        if (seenIds.has(id))
            throw new PendingAskValidationError(`Duplicate question id ${id}`);
        seenIds.add(id);
...
function validateQuestion(question, id) {
    ...
    const options = question.options.map((option) => {
        ...
        if (seenValues.has(value))
            throw new PendingAskValidationError(`Duplicate option value ${value} in question ${id}`);
...
function validateOption(option, value, questionId) {
...
function validateSubmission(ask, submission) {
    ...
    for (const answer of submission.answers) {
        ...
        if (question === undefined)
            throw new PendingAskValidationError(`Unknown question id ${answer.id}`);
        if (answers.has(answer.id))
            throw new PendingAskValidationError(`Duplicate answer for question ${answer.id}`);
        ...
        if (validated !== undefined)
            answers.set(answer.id, validated);
    }
    return answers;
}
function validateAnswer(question, answer) {
    ...
    for (const value of answer.values) {
        if (!optionValues.has(value))
            throw new PendingAskValidationError(`Question ${question.id} has no option ${value}`);
        if (values.includes(value))
            throw new PendingAskValidationError(`Duplicate value ${value} for question ${question.id}`);
        values.push(value);
    }
...
function normalizeOtherText(question, otherText) {
...
function requireId(value, field) {
    if (value.trim() === "")
        throw new PendingAskValidationError(`${field} must not be empty`);
    if (value.length > ASK_USER_ID_MAX_LENGTH)
        throw new PendingAskValidationError(`${field} exceeds its length limit`);
    return value;
}
function requireText(value, field) {
    if (value.trim() === "")
        throw new PendingAskValidationError(`${field} must not be empty`);
    if (value.length > ASK_USER_TEXT_MAX_LENGTH)
        throw new PendingAskValidationError(`${field} exceeds its length limit`);
    return value;
}
//# sourceMappingURL=pendingAskStore.js.map