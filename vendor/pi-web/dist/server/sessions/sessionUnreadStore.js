...
import { SESSION_UNREAD_CATALOG_ID_MAX_LENGTH, SESSION_UNREAD_COMPLETED_AT_MAX_LENGTH, SESSION_UNREAD_CWD_MAX_LENGTH, SESSION_UNREAD_LIMIT, SESSION_UNREAD_SESSION_ID_MAX_LENGTH, } from "../../shared/apiTypes.js";
...
export class SessionUnreadStore {
    ...
    observeActivityState(sessionId, cwd, active) {
        this.requireLoaded();
        ...
        if (!this.activeByIdentity.has(key))
            return [];
    ...
    reconcileCwd(cwd, sessionIds) {
        ...
        for (const [key, identity] of this.activeByIdentity) {
            if (identity.cwd === boundedCwd && !retained.has(identity.sessionId))
                this.activeByIdentity.delete(key);
        }
    ...
    async flush() {
        ...
        if (this.persistenceFailure === undefined
            && this.persistenceGeneration === this.durablePersistenceGeneration)
            return;
    ...
    async loadPersistedState() {
        ...
        try {
            ...
            if (!(error instanceof SessionUnreadPersistenceCorruptionError))
                throw error;
            resetState = true;
        }
    ...
    ensurePersistenceWorker() {
        if (this.persistence === undefined || this.persistenceWorker !== undefined || this.pendingPersistence === undefined)
            return;
        const worker = this.runPersistenceWorker();
        this.persistenceWorker = worker;
        void worker.finally(() => {
            if (this.persistenceWorker === worker)
                this.persistenceWorker = undefined;
        });
    }
    async runPersistenceWorker() {
        const persistence = this.persistence;
        if (persistence === undefined)
            return;
    ...
    async waitForPersistenceWorker() {
    ...
    requireLoaded() {
        if (!this.loaded)
            throw new Error("Session unread store must be loaded before use");
    }
...
export class FileSessionUnreadPersistence {
    constructor(filePath = defaultSessionUnreadFilePath()) {
...
export function defaultSessionUnreadFilePath(env = process.env, cwd = process.cwd()) {
...
function parsePersistedState(value) {
    ...
    if (record["version"] !== SESSION_UNREAD_STATE_VERSION)
        throw new Error("Unsupported session unread state version");
    ...
    if (!Array.isArray(rawSessions))
        throw new Error("Session unread sessions must be an array");
    if (rawSessions.length > SESSION_UNREAD_LIMIT)
        throw new Error("Session unread state exceeds its session limit");
    ...
    for (const summary of sessions) {
        ...
        if (orders.has(summary.completionOrder))
    ...
    if (nextCompletionOrder < maxOrder)
        throw new Error("Session unread completion order is inconsistent");
    if (catalogRevision < nextCompletionOrder)
        throw new Error("Session unread catalog revision is inconsistent");
    return {
        version: SESSION_UNREAD_STATE_VERSION,
        catalogId,
        catalogRevision,
...
function parseSummary(value) {
    ...
    const completedAt = requireBoundedNonEmptyString(record["completedAt"], "completedAt", SESSION_UNREAD_COMPLETED_AT_MAX_LENGTH);
    ...
    return {
        sessionId: requireBoundedNonEmptyString(record["sessionId"], "sessionId", SESSION_UNREAD_SESSION_ID_MAX_LENGTH),
...
function requireNonEmptyString(value, field) {
    if (typeof value !== "string" || value === "")
        throw new Error(`Session unread ${field} must be a non-empty string`);
    return value;
}
function requireBoundedNonEmptyString(value, field, maxLength) {
    const parsed = requireNonEmptyString(value, field);
    if (parsed.length > maxLength)
        throw new Error(`Session unread ${field} exceeds its length limit`);
    return parsed;
}
function requireCatalogId(value) {
...
function requirePositiveSafeInteger(value, field) {
    ...
    if (parsed === 0)
        throw new Error(`Session unread ${field} must be positive`);
    return parsed;
}
...
function sessionIdentityKey(identity) {
...
//# sourceMappingURL=sessionUnreadStore.js.map