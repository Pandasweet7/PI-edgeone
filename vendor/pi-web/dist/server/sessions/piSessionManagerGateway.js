...
export function createPiSessionManagerGateway(options) {
...
class SettingsAwarePiSessionManagerGateway {
    constructor(resolver) {
         ...
         */
        this.summaryScanner = new SessionSummaryScanner();
    }
    async list(cwd) {
...
export async function listSessionsInDir(sessionDir) {
...
export async function listSessionsInDefaultPiStore(storeRoot) {
    let entries;
    try {
...
export function filterSessionsForCwd(sessions, cwd) {
 ...
 */
export async function resolveSessionFileInDir(sessionDir, cwd, sessionId, readHeader) {
    ...
    for (const sessionFile of [...fileNameMatches, ...remainingFiles]) {
        ...
        if (header?.cwd === undefined)
            continue;
        if (!cwdPathsEqual(header.cwd, cwd))
            continue;
 ...
 */
function byNewestEmbeddedTimestamp(a, b) {
...
function embeddedFileNameTimestamp(fileName) {
...
function fileNameMatchesSessionId(fileName, sessionId) {
 ...
 */
function embeddedFileNameSessionId(fileName) {
    const stem = fileName.slice(0, -".jsonl".length);
    const separatorIndex = stem.indexOf("_");
...
//# sourceMappingURL=piSessionManagerGateway.js.map