...
export async function computeEditPreview(path, edits, cwd) {
...
function applyEditsToNormalizedContent(normalizedContent, edits, path) {
    ...
    for (let index = 0; index < normalizedEdits.length; index++) {
        if ((normalizedEdits[index]?.oldText ?? "") === "")
            throw editError("empty", path, index, normalizedEdits.length);
    }
    ...
    for (let index = 0; index < normalizedEdits.length; index++) {
        ...
        if (!match.found)
            throw editError("missing", path, index, normalizedEdits.length);
        const occurrences = countOccurrences(baseContent, edit.oldText);
        if (occurrences > 1)
            throw editError("duplicate", path, index, normalizedEdits.length, occurrences);
    ...
    if (baseContent === newContent)
        throw editError("nochange", path, 0, normalizedEdits.length);
    return { baseContent, newContent };
}
function editError(kind, path, editIndex, totalEdits, occurrences) {
    const prefix = totalEdits === 1 ? "" : `edits[${String(editIndex)}].`;
    if (kind === "empty")
        return new Error(totalEdits === 1 ? `oldText must not be empty in ${path}.` : `${prefix}oldText must not be empty in ${path}.`);
    if (kind === "missing")
        return new Error(totalEdits === 1 ? `Could not find the exact text in ${path}. The old text must match exactly including all whitespace and newlines.` : `Could not find edits[${String(editIndex)}] in ${path}. The oldText must match exactly including all whitespace and newlines.`);
    if (kind === "duplicate")
        return new Error(totalEdits === 1 ? `Found ${String(occurrences)} occurrences of the text in ${path}. The text must be unique. Please provide more context to make it unique.` : `Found ${String(occurrences)} occurrences of edits[${String(editIndex)}] in ${path}. Each oldText must be unique. Please provide more context to make it unique.`);
...
function appendContextLines(output, lines, lineNumWidth, nextOldLine, nextNewLine) {
