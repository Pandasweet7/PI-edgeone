 ...
 */
export function projectSessionTree(roots, activeLeafId) {
    const nodes = [];
    ...
    while (stack.length > 0) {
        ...
        if (candidate === undefined)
            continue;
        if (!isProjectableSessionTreeNode(candidate))
            throw new Error("Pi returned a malformed session-tree node");
        ...
        if (projectedIds.has(node.entry.id))
            throw new Error("Pi returned duplicate session-tree entry IDs");
...
function projectNode(node, entry) {
...
function projectCustomMessage(value) {
    if (value["display"] !== true)
        return { kind: "custom-message", summary: "Hidden custom message" };
...
function assistantSummary(message) {
    ...
    if (text !== "")
        return summary(text, "Assistant response");
    const toolNames = assistantToolNames(message["content"]);
    if (toolNames.length > 0)
        return summary(`Tool call: ${toolNames.join(", ")}`, "Assistant tool call");
    if (message["stopReason"] === "error")
        return "Assistant error";
    if (message["stopReason"] === "aborted")
        return "Assistant response aborted";
    return "Assistant response";
}
function assistantToolNames(content) {
...
function modelChangeSummary(entry) {
    ...
    if (provider !== undefined && modelId !== undefined)
        return summary(`Model: ${provider}/${modelId}`, "Model changed");
    return namedSummary("Model", modelId ?? provider, "Model changed");
}
function contentPreview(content, includeImageMarkers) {
    const fragments = [];
    ...
    if (typeof content === "string") {
        ...
        for (const part of content) {
            ...
            if (part["type"] === "text" && typeof part["text"] === "string")
                append(part["text"]);
            else if (includeImageMarkers && part["type"] === "image")
                append("[image]");
            if (remaining <= 0)
                break;
        }
...
function namedSummary(prefix, value, fallback) {
...
function buildActivePath(nodes, activeLeafId) {
...
function isProjectableSessionTreeNode(value) {
    return isRecord(value) && Array.isArray(value["children"]);
}
function isProjectableSessionEntry(value) {
...
//# sourceMappingURL=sessionTreeProjection.js.map