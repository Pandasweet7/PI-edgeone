...
const ReadSubsessionParams = Type.Object({
    ...
    roles: Type.Optional(Type.Array(Type.Union([Type.Literal("assistant"), Type.Literal("user"), Type.Literal("tool"), Type.Literal("system"), Type.Literal("custom")]), { description: "Roles to include; omit for all." })),
    include: Type.Optional(Type.Array(Type.Union([Type.Literal("text"), Type.Literal("thinking"), Type.Literal("tool_call"), Type.Literal("tool_result"), Type.Literal("image")]), { description: "Content kinds to include; omit for all." })),
...
function workingInspectionGuidance(sessionId) {
...
function renderPart(part) {
    if (part.kind === "text")
        return `${part.text}${clipNotice(part)}`;
    if (part.kind === "thinking")
        return `[thinking] ${part.text}${clipNotice(part)}`;
    ...
    if (part.kind === "tool_result")
        return `[result${part.isError ? " error" : ""}${part.toolName === undefined ? "" : ` ${part.toolName}`}] ${part.text}${clipNotice(part)}`;
    return "[image]";
}
 ...
 */
export function createSubsessionToolDefinitions(spawningCwd, deps) {
...
//# sourceMappingURL=spawnSubsessionTool.js.map