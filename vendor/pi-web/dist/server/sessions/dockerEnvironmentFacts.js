...
const DOCKER_SOCKET_MOUNTS = ["/run/docker.sock", "/var/run/docker.sock"];
/**
 ...
 */
const CONTAINER_INTERNAL_TARGET_ROOTS = ["/proc", "/sys", "/dev", "/etc", "/run", "/var/run", "/tmp"];
/**
 ...
 */
export function readContainerMounts(path = MOUNT_INFO_PATH) {
 ...
 */
export function parseContainerMounts(mountInfo) {
    const mounts = [];
    for (const line of mountInfo.split("\n")) {
        ...
        if (target === undefined || options === undefined || fsType === undefined || fsType === "")
            continue;
        mounts.push({
 ...
 */
export function dockerEnvironmentFacts({ env, mounts }) {
    if (!isDockerDeployment(env))
        return undefined;
 ...
 */
export function dockerEnvironmentPromptSections(options) {
    const { env, enabled, logger, readMountInfo } = options;
    if (!enabled || !isDockerDeployment(env))
        return [];
    let mounts;
    try {
        ...
        logger?.warn({ err: error, path: MOUNT_INFO_PATH }, "could not read the container mount table; Docker environment facts are omitted from session system prompts");
        return [];
    }
    const facts = dockerEnvironmentFacts({ env, mounts });
    return facts === undefined ? [] : [facts];
}
function isDockerDeployment(env) {
    const value = env["PI_WEB_DOCKER_RUNTIME"];
    return value !== undefined && value !== "" && value !== "0" && value.toLowerCase() !== "false";
}
function persistentDataFact(mounts, env) {
    if (!mounts.some((mount) => mount.target === DATA_MOUNT))
        return [];
 ...
 * location.
 */
function hostSharedMountTargets(mounts) {
...
function dockerSocketFact(mounts) {
...
function devWorkspaceFacts(mounts, devRepoRoot) {
    if (!mounts.some((mount) => mount.target === DEV_WORKSPACE_MOUNT))
        return [];
...
function isHostPathMount(mount) {
    if (mount.target === "/" || !mount.target.startsWith("/"))
        return false;
    if (CONTAINER_INTERNAL_FS_TYPES.has(mount.fsType))
        return false;
...
function unescapeMountField(value) {
    return value.replace(/\\(040|011|012|134)/g, (_match, code) => String.fromCharCode(Number.parseInt(code, 8)));
}
function nonEmptyEnv(env, key) {
    const value = env[key];
    return value === undefined || value === "" ? undefined : value;
}
//# sourceMappingURL=dockerEnvironmentFacts.js.map