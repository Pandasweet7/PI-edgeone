...
export class SessionSummaryScanner {
     ...
     */
    async scanSessionSummariesInDir(sessionDir) {
    ...
    pruneEntriesRemovedFrom(sessionDir, existingFiles) {
        ...
        for (const path of this.memo.keys()) {
            ...
            if (path.startsWith(dirPrefix) && !existing.has(path))
                this.memo.delete(path);
        }
    }
    async scanFileWithMemo(filePath, chunkBuffer) {
    ...
    async fullScan(filePath, chunkBuffer) {
 ...
 * such files too.
 */
async function scanWholeSessionFile(filePath, chunkBuffer) {
 ...
 */
async function foldFileLines(file, fold, chunkBuffer) {
    let position = 0;
    let pendingChunks = [];
    for (;;) {
...
function buildSummaryFromFold(fold, filePath, mtime) {
    if (fold.rejected || fold.header === undefined)
        return undefined;
...
function processLineBytes(data, start, end, state) {
    ...
    if (entryType === "session_info") {
        ...
        if (entry !== undefined)
            state.name = sessionInfoName(entry);
        return;
    }
    if (entryType === "message") {
        ...
        if (!endsWithClosingBrace(data, start, end))
            return;
        ...
        if (entry !== undefined) {
            ...
            if (userText !== undefined)
                state.firstMessageText = userText;
        }
        return;
    }
 ...
 */
function classifyLineType(data, start, end) {
    const prefixLength = ENTRY_TYPE_PREFIX.length;
    if (end - start < prefixLength + 1)
        return undefined;
    for (let i = 0; i < prefixLength; i += 1) {
        if (data[start + i] !== ENTRY_TYPE_PREFIX[i])
            return undefined;
    }
    ...
    if (closeAt === -1 || closeAt > searchLimit)
        return undefined;
    if (sameBytes(data, start + prefixLength, closeAt, MESSAGE_TYPE_BYTES))
        return "message";
    if (sameBytes(data, start + prefixLength, closeAt, SESSION_INFO_TYPE_BYTES))
        return "session_info";
    return "other";
}
/** Whether `data[start, end)` holds exactly `expected`'s bytes. */
function sameBytes(data, start, end, expected) {
 ...
 */
function endsWithClosingBrace(data, start, end) {
    let last = end;
    while (last > start) {
 ...
 */
function classifyPreHeaderLine(line) {
...
function sessionInfoName(entry) {
...
async function listSessionFilesInDir(sessionDir) {
    let fileNames;
    try {
...
function sortedSessionSummaries(summaries) {
    const sessions = summaries.filter((summary) => summary !== undefined);
    sessions.sort((a, b) => b.modified.getTime() - a.modified.getTime());
    return sessions;
}
async function scanSessionFilesWithBoundedConcurrency(files, chunkBytes, scan) {
    const results = Array.from({ length: files.length }, () => undefined);
    let nextIndex = 0;
    ...
    const workers = Array.from({ length: workerCount }, async () => {
        ...
        // never happen.
        let chunkBuffer;
        const readBuffer = () => (chunkBuffer ??= Buffer.allocUnsafe(chunkBytes));
        for (;;) {
...
//# sourceMappingURL=sessionSummaryScanner.js.map