...
async function loadGlobalExtensionServices(runtime, agentDir) {
...
function logBootstrapDiagnostic(logger, diagnostic) {
    const details = {
        context: LOG_CONTEXT,
...
function freezeProviderMutations(runtime, logger, configBaseline) {
    ...
    const logQuietly = (details, message) => {
        try {
    ...
    const logIgnoredMutation = (operation, providerId) => {
        const loggedIds = loggedProviderIds[operation];
        if (loggedIds.has(providerId))
            return;
    ...
    const frozenMethods = {
        registerProvider(providerId, config) {
            ...
            if (accepted)
                configBaseline.set(providerId, accepted);
            logQuietly({
                context: LOG_CONTEXT,
 ...
 */
export async function bootstrapAndFreezeGlobalExtensionProviders(runtime, agentDir, logger) {
    ...
    for (const diagnostic of services.diagnostics)
        logBootstrapDiagnostic(logger, diagnostic);
    for (const extensionError of services.resourceLoader.getExtensions().errors) {
        logger.error({ context: LOG_CONTEXT, error: extensionError.error }, "global extension failed during provider bootstrap");
    }
    logger.info({ context: LOG_CONTEXT, providerIds }, "global extension provider baseline bootstrapped and frozen");
}
//# sourceMappingURL=globalProviderPolicy.js.map