...
export function deterministicSessionName(firstMessage) {
    if (typeof firstMessage !== "string")
        return undefined;
    return relayHandoffSessionName(firstMessage.trimStart());
}
export async function generateShortSessionName(streamFn, model, firstMessage) {
...
function relayHandoffSessionName(firstMessage) {
    ...
    if (relayName === undefined || relayName === "" || legIdentifier === undefined)
        return undefined;
    return cleanSessionName(formatRelaySessionName(relayName, legIdentifier));
}
function formatRelaySessionName(relayName, legIdentifier) {
    const prefix = "Relay ";
...
function truncateRelayName(relayName, maxLength) {
    if (relayName.length <= maxLength)
        return relayName;
