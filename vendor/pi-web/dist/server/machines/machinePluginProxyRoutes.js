...
function remotePluginModulePath(pluginId, module) {
    ...
    try {
        ...
        if (url.origin !== pluginRootUrl.origin || !url.pathname.startsWith(prefix))
            return undefined;
...
function parseRemoteManifest(value) {
    ...
    for (const plugin of plugins) {
        if (ids.has(plugin.id))
            throw new Error(`Duplicate remote PI WEB plugin id: ${plugin.id}`);
        ids.add(plugin.id);
    }
    return { lifecycleVersion, plugins };
}
function parseRemoteLifecycleVersion(value) {
    if (value === undefined) {
...
class RemotePluginLifecycleCompatibilityError extends Error {
    constructor() {
        super(...arguments);
        this.name = "RemotePluginLifecycleCompatibilityError";
    }
}
function parseRemoteBackendRevision(value) {
    if (value === undefined)
        return {};
    try {
...
function parseRemoteMachineSpecific(value) {
    if (value === undefined)
        return {};
...
function sendLifecycleCompatibilityError(reply, machineId, detail) {
...
function formatUnknownValue(value) {
    if (typeof value === "string" || typeof value === "number" || typeof value === "boolean" || typeof value === "bigint" || typeof value === "symbol" || typeof value === "function" || value === null || value === undefined)
        return String(value);
    try {
