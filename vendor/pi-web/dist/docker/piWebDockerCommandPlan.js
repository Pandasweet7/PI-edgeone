...
const commands = new Set([...PI_WEB_DOCKER_USER_COMMANDS, "help"]);
const logsTargets = new Set(["web", "sessiond", "data-init"]);
const shellTargets = new Set(["web", "sessiond"]);
export function piWebDockerCommandPrefix(mode) {
    return mode === "dev" ? "pi-web-docker --dev" : "pi-web-docker";
}
export function piWebDockerCommand(mode, command) {
    return `${piWebDockerCommandPrefix(mode)} ${command}`;
}
export function planPiWebDockerRuntimeHostCommand(plan) {
...
export function planPiWebDockerDevHostCommand(plan) {
    if (plan.mode !== "dev")
        return undefined;
    switch (plan.command) {
        case "install":
...
export function validatePiWebDockerDevRootSafety(plan, uid) {
    if (plan.mode !== "dev" || plan.allowRoot || plan.command === "help" || uid !== 0)
        return undefined;
    return "refusing to run Docker development mode as root; retry with --allow-root if this is intentional";
}
export function parsePiWebDockerArgs(argv) {
    let mode = "runtime";
    let allowRoot = false;
    let index = 0;
    ...
    if (commandValue === undefined)
        return { ok: false, errors: ["Missing command"] };
    if (!isPiWebDockerCommand(commandValue))
        return { ok: false, errors: [`Unknown command: ${commandValue}`] };
    const args = argv.slice(index + 1);
    const plan = { mode, command: commandValue, allowRoot, args };
    return validatePlan(withDefaultTarget(plan));
}
function composeHostPlan(...args) {
    return { kind: "compose", args };
}
function devComposeHostPlan(...args) {
    return { kind: "compose", args, usesGeneratedEnv: true };
}
function withDefaultTarget(plan) {
    if (plan.command === "shell" && plan.target === undefined && plan.args.length === 0)
        return { ...plan, target: "web" };
    return plan;
}
function validatePlan(plan) {
    ...
    if (plan.command === "logs") {
        ...
        if (plan.args[0] === "data-init" && plan.mode !== "dev")
            errors.push("logs data-init is only available in development mode");
    }
...
function validateOptionalTarget(args, isAllowed, command, errors) {
    if (args.length > 1) {
        errors.push(`${command} accepts at most one target`);
        return;
    }
    const [target] = args;
    if (target !== undefined && !isAllowed(target))
        errors.push(`Invalid ${command} target: ${target}`);
}
function targetFrom(command, args) {
    const [target] = args;
    if (target === undefined)
        return command === "shell" ? "web" : undefined;
    if (command === "logs" && isLogsTarget(target))
        return target;
    if (command === "shell" && isShellTarget(target))
        return target;
    return undefined;
}
function isPiWebDockerCommand(value) {
    return commands.has(value);
}
function isLogsTarget(value) {
    return logsTargets.has(value);
}
function isShellTarget(value) {
    return shellTargets.has(value);
}
//# sourceMappingURL=piWebDockerCommandPlan.js.map