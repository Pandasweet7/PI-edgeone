// Generated from pi-web-plugins/git/browser/unifiedDiff.ts. Do not edit directly.
...
function line(kind, prefix, text, numbers = {}) {
    return {
        kind,
        prefix,
        text,
...
function applyInlineDiffs(lines) {
    let index = 0;
...
function applyInlineDiffBlock(removedLines, addedLines) {
    if (removedLines.length + addedLines.length > maxInlineBlockLines)
        return;
...
function pairChangedLines(removedLines, addedLines) {
    if (removedLines.length === addedLines.length)
        return removedLines.map((removed, index) => ({ removed, added: addedLines[index] })).filter(isCompletePair);
    if (removedLines.length === 1)
        return bestPairsForSingleRemovedLine(removedLines[0], addedLines);
    if (addedLines.length === 1)
        return bestPairsForSingleAddedLine(removedLines, addedLines[0]);
    const pairs = [];
    ...
    for (let index = 0; index < pairCount; index++) {
        ...
        if (removed === undefined || added === undefined)
            continue;
        if (lineSimilarity(removed.text, added.text) >= minPairSimilarity)
...
function computeInlineDiff(oldText, newText) {
    if (oldText === newText)
        return undefined;
    if (oldText.length > maxInlineLineLength || newText.length > maxInlineLineLength)
        return undefined;
    const oldUnits = splitTextUnits(oldText);
    const newUnits = splitTextUnits(newText);
    if (oldUnits.length > maxInlineLineLength || newUnits.length > maxInlineLineLength)
        return undefined;
    ...
    if (Math.max(oldUnits.length, newUnits.length) >= 20 && similarity < minInlineSimilarity)
        return undefined;
    const removed = [];
    const added = [];
    ...
    if (!removed.some((span) => span.changed) && !added.some((span) => span.changed))
        return undefined;
...
function similarityFromChanges(changes, oldLength, newLength) {
 ...
 */
function diffTextUnits(oldText, newText) {
    if (sameTextUnits(oldText, newText))
        return oldText.length === 0 ? [] : [{ value: oldText.join(""), unitCount: oldText.length }];
    let prefixLength = 0;
    ...
    if (oldMiddle.length * newMiddle.length > maxInlineMatrixCells) {
        ...
        for (const change of diffMiddleCharacters(oldMiddle, newMiddle))
            appendCharacterChange(changes, splitTextUnits(change.value), change.added === true ? "added" : change.removed === true ? "removed" : undefined);
    }
    appendCharacterChange(changes, oldText.slice(oldText.length - suffixLength));
    return changes;
}
function diffMiddleCharacters(oldText, newText) {
    if (oldText.length === 0)
        return newText.length === 0 ? [] : [{ value: newText.join(""), unitCount: newText.length, added: true }];
    if (newText.length === 0)
        return [{ value: oldText.join(""), unitCount: oldText.length, removed: true }];
...
function appendCharacterChange(changes, units, kind) {
    if (units.length === 0)
        return;
...
function sameTextUnits(left, right) {
...
function mergeAdjacentSpans(spans) {
    ...
    for (const span of spans) {
        ...
        if (previous?.changed === span.changed)
